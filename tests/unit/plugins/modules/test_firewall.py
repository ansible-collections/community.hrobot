...
def run_module(mocker, module, arguments, fetch_url):
    ...
    mocker.patch('ansible_collections.community.hrobot.plugins.module_utils.robot.fetch_url', fetch_url)
    mocker.patch('ansible_collections.community.hrobot.plugins.module_utils.robot.time.sleep', lambda duration: None)
    mocker.patch('ansible_collections.community.hrobot.plugins.modules.firewall.AnsibleModule.exit_json', exit_json)
    mocker.patch('ansible_collections.community.hrobot.plugins.modules.firewall.AnsibleModule.fail_json', fail_json)
...
def test_absent_idempotency(mocker):
    result = run_module_success(mocker, firewall, {
        'hetzner_user': '',
        'hetzner_password': '',
        'server_ip': '1.2.3.4',
        'state': 'absent',
...
def test_wait_get_timeout(mocker):
    result = run_module_failed(mocker, firewall, {
        'hetzner_user': '',
        'hetzner_password': '',
        'server_ip': '1.2.3.4',
        'state': 'present',
