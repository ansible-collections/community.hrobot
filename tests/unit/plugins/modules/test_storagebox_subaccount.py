...
class TestHetznerStorageboxSubbacount(BaseTestModule):
    ...
    def test_update_subaccount_invalid_password(self, mocker):
        ...
        assert result['msg'] == "Subaccount with username 'u2342-sub1' and homedirectory 'test' has an invalid password (says Hetzner)"

    def test_invalid_spec_delete_in_username(self, mocker):
        result = self.run_module_failed(
            mocker,
            storagebox_subaccount,
            {
        ...
        assert result['msg'] == "username is required when state is 'absent'"
